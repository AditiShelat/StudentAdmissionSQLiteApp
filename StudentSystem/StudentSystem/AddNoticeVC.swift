...
class AddNoticeVC: UIViewController {
    ...
    func update(not: Notice){
        SQLiteHandlerNotice.shared.update(not: not){ success in
            if success{
                let alertController = UIAlertController(title: "Update Notice", message: "updated successfully", preferredStyle: .alert)
    ...
    func insert(not: Notice){
        SQLiteHandlerNotice.shared.insert(not: not){ success in
            if success{
                let alertController = UIAlertController(title: "Insert Notice", message: "inserted successfully", preferredStyle: .alert)
                let OKAction = UIAlertAction(title: "OK", style: .default) {
                    (action: UIAlertAction!) in
                    print("Ok button tapped");
                }
